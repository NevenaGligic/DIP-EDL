...
def load_model(ID_dataset, pretrained, index, dropout_rate, device,
               num_classes=None, input_len=None):
    if ID_dataset == "MNIST":
        if pretrained :
            model = conv_net()
    ...
    if ID_dataset == "LAMOST":
        out_dim  = num_classes if num_classes is not None else 2
        feat_len = input_len   if input_len   is not None else 556
        model = conv1d_net_lamost(num_classes=out_dim, input_len=feat_len)
...
# ── 1-D spectral backbone (LAMOST) ───────────────────────────────────────────
# Mirrors ConvLinSeq / conv_net but uses Conv1d + MaxPool1d instead of the
# 2-D equivalents, making it directly applicable to spectral time-series data.

def convolution_sequential_1d(input_len, channel_dim, hidden_dims, kernel_dim,
                               k_lipschitz=None, p_drop=None):
    dims = [channel_dim] + hidden_dims
    layers = []
    for i in range(len(hidden_dims)):
        pad = (kernel_dim - 1) // 2
        conv = nn.Conv1d(dims[i], dims[i + 1], kernel_dim, padding=pad)
        if k_lipschitz is not None:
...
class ConvLinSeq1d(nn.Module):
    """1-D analogue of ConvLinSeq for spectral data (e.g. LAMOST).

    Sets self.feature (flattened conv output) on every forward pass, which is
    the side-effect required by DAEDL's fit_gda / gmm_forward functions.
    """

    def __init__(self, input_len, channel_dim, conv_hidden_dims, linear_hidden_dims,
                 output_dim, kernel_dim, k_lipschitz=None, p_drop=None):
        super().__init__()
        if k_lipschitz is not None:
            k_lipschitz = k_lipschitz ** (1. / 2.)

        self.convolutions = convolution_sequential_1d(
            input_len=input_len,
        ...
        # Compute the flattened length after 3× MaxPool1d(4)
        flat_len = conv_hidden_dims[-1]
        cur_len = input_len
        for _ in conv_hidden_dims:
            cur_len = cur_len // 4
        flat_len = conv_hidden_dims[-1] * cur_len
    ...
    def forward(self, x):
        batch_size = x.size(0)
        x = self.convolutions(x)                          # (B, C, L')
        self.feature = x.clone().detach().reshape(batch_size, -1)
        x = self.linear(x.reshape(batch_size, -1))
        return x


def conv1d_net_lamost(num_classes=2, input_len=556):
    """Spectral-norm Conv1d backbone for LAMOST (input: B×1×input_len)."""
    return ConvLinSeq1d(
        input_len=input_len,
        channel_dim=1,
        conv_hidden_dims=[64, 64, 64],
